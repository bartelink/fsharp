...
* Add `Async.StartTaskImmediate`, which propagates the ambient `Async.CancellationToken` to a task factory, then awaits the result using `Async.Await` semantics. Overloads for `Task`, `Task<'T>`, `ValueTask`, `ValueTask<'T>` and task-like `.GetAwaiter()` (via SRTP). ([Language Suggestion #1284](https://github.com/fsharp/fslang-suggestions/issues/1284))
